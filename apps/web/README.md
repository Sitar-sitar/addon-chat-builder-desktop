...
IDは形式だけを検証し、Minecraft内での実在確認は行いません。採掘・死亡イベントは最大1秒遅延します。ブロックドロップ差し替えは元のloot tableを完全上書きするため、シルクタッチ・幸運の元挙動は失われます。時刻・天候の固定アクションは定期実行トリガーでのみ使えます（周期再設定で近似固定するため）。

正本は親リポの `docs/機能設計書_Java多パターン対応_2026-07-19.md`。リリース後の修正は `docs/修正設計書_Java多パターン実装精査対応_2026-07-20.md`（v0.7.1）と `docs/修正設計書_Java多パターン現行再評価対応_2026-07-23.md`（v0.7.2）で追補します。capabilityの網羅は「ID登録漏れ＝`Record<JavaCapabilityId, true>` の網羅定数」＋「生成内容＝版×capabilityテスト」で担保します（機能設計書の `Record<…, GeneratorFn>` 完全マップ記述はv0.7.2でsupersede）。
