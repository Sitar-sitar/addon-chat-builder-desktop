...
Java版 apps/web v0.7.0 では、レシピ8種、定期・イベント駆動スクリプト、条件付きアクション、ブロックドロップ差し替え、lang／アイテムモデル差し替えを決定論的に生成する。対象バージョンは `JAVA_TARGET_VERSION` で厳密に指定し、`1.21`、`1.21.4`、`1.21.5`、`1.21.7`、`26.2` に対応する。このPCの実機確認用設定は `1.21.5`、未設定時の既定値は `1.21.7`。
